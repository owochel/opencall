...
const rows = schedule.map(item => `<article class="schedule-row"><div class="week-label"><strong>Week ${item.week}</strong><span>${item.date}</span></div><div><h3>${item.theme}</h3>${item.question ? `<p>${item.question}</p>` : ""}${item.event ? `<a class="schedule-event-link" href="${item.event.href}">${item.event.title}</a>` : ""}</div><div><h4>In-class activity</h4>${activityList(item.activity)}</div><div><h4>Due on this day</h4><p>${item.due}</p>${readingLinks(item.readings)}</div></article>`).join("");
