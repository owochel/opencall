...
export default function Readings() {
  ...
  return <main className="site-shell">
    ...
    <div className="reading-layout">
      ...
      <div>{weeksWithReadings.map(item=><section className="reading-group" key={item.week}><h2>Week {item.week}</h2><ol className="reading-list">{item.readings!.map(reading=><li key={reading.title}><a href={reading.href}>{reading.title}{reading.optional ? " (Optional)" : ""}</a><span>{reading.type}{reading.optional ? " · OPTIONAL" : ""}</span></li>)}</ol></section>)}</div>
    </div>
  </main>;
}
